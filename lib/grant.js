...
function extractCredentials (done) {
  ...
  } else if (!this.client.clientSecret && this.grantType !== 'password') {
...
function generateRefreshToken (done) {
  ...
  if (this.refreshToken) return done();

  var self = this;

  this.model.getRefreshTokenByUserIdClientId(self.user.id, self.client.clientId, function(err, refreshToken) {
    if (err) return done(err);
    if (refreshToken) {
      self.refreshToken = refreshToken;
      return done();
    }
    token(self, 'refreshToken', function (err, token) {
